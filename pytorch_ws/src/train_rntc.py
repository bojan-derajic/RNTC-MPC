"""Training script for the Residual Neural Terminal Constraint (RNTC) model.

The RNTC model uses a hypernetwork to generate weights for a main MLP that
predicts a *residual* correction to the signed-distance-function (SDF) term,
yielding the full value function::

    vf_pred = sdf_term - residual_pred

where ``sdf_term`` is channel 0 of the SDF input (the current-time SDF slice)
and ``residual_pred`` is the main network output.
...
def _log_confusion_matrix(
    ...
    with torch.no_grad():
        for sdf, vf in test_dataloader:
            ...
            res_pred = main_net(grid_flatten).view_as(vf)
            sdf_term = sdf[:, 0, ..., None]
            vf_pred = sdf_term - res_pred
            batches.append(conf_mat_func(vf_pred, vf))

    # Average confusion matrix across all test batches
...
def train(
    ...
    """Train the RNTC hypernetwork model.
    ...
    Args:
        ...
        sdf_idx:          List of SDF channel indices fed to the hypernetwork.
                          Channel 0 is always used as the SDF term for the
    ...
    mlflow.set_experiment(f"RNTC ({dynamics})")

    with mlflow.start_run(run_name=loss_func_name):
        # Log source code as artifact for reproducibility
        ...
        for epoch in range(num_epochs):
            ...
            for sdf, vf in train_loader:
                # Hypernetwork produces main-network parameters from SDF channels
                main_net_params = hypernet(sdf[:, sdf_idx, ...])
                main_net.set_params(main_net_params)

                # Residual formulation: vf = sdf_term − residual
                res_pred = main_net(grid_flat).view_as(vf)
                sdf_term = sdf[:, 0, ..., None]   # channel 0 = current-time SDF
                vf_pred  = sdf_term - res_pred

                # Warm-start: use plain MSE for the first epoch to stabilise
                # training before switching to CME (which has a large exp term).
            ...
            # Checkpoint weights every 5 epochs
            if epoch % 5 == 0:
        ...
        with torch.no_grad():
            ...
            for vis_num, test_idx in enumerate(vis_indices):
                ...
                sdf_plot  = sdf_term.cpu()[..., 0]   # (H, W) or similar 2-D slice
                vf_plot   = vf.cpu()[..., 0]
...
if __name__ == "__main__":
    ...
    # Main network architecture: SIREN-style layers (sin) followed by
    # ReLU layers, ending in a Softplus output (non-negative residual).
    main_net_config = {
        "input_size": 3,          # 3-D grid coordinates (x, y, θ)
        "layers": [
            (36, "sin"),          # SIREN block — learns periodic features
            (36, "sin"),
            (36, "sin"),
            (18, "relu"),         # ReLU block — refines representation
            (18, "relu"),
            (18, "relu"),
            (9, "relu"),
            (9, "relu"),
            (9, "relu"),
            (1, "softplus"),      # Non-negative scalar residual output
        ],
    }

    # Learning-rate schedule: decay by ×0.1 at epochs 85 and 95
    lr_sched_config = {
        "milestones": [85, 95],
        "gamma": 0.1,
    }
